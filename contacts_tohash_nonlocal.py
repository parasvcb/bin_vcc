...
def contacts_computer(has,dcd,pdb):
    ...
    if int(contype)==0:
        ...
        con=mdtraj.compute_contacts(t, contacts='all', scheme='closest-heavy', ignore_nonprotein=True)
    else:
        print('computing CA-CA distance')
        con=mdtraj.compute_contacts(t, contacts='all', scheme='ca', ignore_nonprotein=True)
    print ("computed")
    object0,object1=con
    if len(has)==0:
        for i in object1:
...
append= "_ca-ca_nonlocal.hash" if int(contype) else "_closest_heavy_nonlocal.hash" 
